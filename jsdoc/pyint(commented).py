try:
    import pygame as pg ###імпортуємо модуль пайгейм
    from tkinter import *
    from tkinter import messagebox
    from tkinter.filedialog import askopenfilename, asksaveasfilename
except:
    import install_requirements ###перехоплюємо помилки інсталяції
    import pygame as pg
    from tkinter import *
    from tkinter import messagebox
    from tkinter.filedialog import askopenfilename, asksaveasfilename
...
sw, sh = 960, 850 ###задаємо розміри вікна
sc = (sw // 2, sh // 2)
screen = pg.display.set_mode((sw, sh))
pg.display.set_caption("Pyint") ###назва вікна
pg.display.set_icon(pg.image.load("icon.png"))###вставляємо іконку

fillImage = pg.transform.scale(pg.image.load("imgs/fill.png"), (40, 40)) ###завантажуємо зображення для інструментів
...
def draw_walls(): ###робимо розмітку екрану
    wall_color = (50, 50, 50)
    wall_thickness = 4

    pg.draw.rect(screen, (150, 150, 150),
                 (g1.xCount * g1.cellSize, 0, sw - g1.xCount * g1.cellSize, g1.yCount * g1.cellSize))
...
class Cell(object):  ###клас для функцій виклику
    ...
    def change_color(self, color): ###зміна кольору
        self.color = color
        self.subsurface.fill(self.color)

    def Draw(self, win, x, y): ###функція малювання
        self.pos = (x, y)
        win.blit(self.subsurface, self.pos)


class Grid(object): ###розташування артборду
    ...
    def clean(self):###очистка артборду
        for i in range(self.xCount):
            for j in range(self.yCount):
                self.grid[i][j].change_color(self.color)


class Button(object):###класс налаштування кнопок
    active = False
    clicked = False
    rollOver = False
    ...
    def Draw(self, win, val=-1):
        if self.type == 1:
            ...
            pg.draw.rect(screen, (140, 140, 140),
                         (self.drawPos[0] - 60, self.drawPos[1] + self.height / 3, 120, self.height / 2))
...
logo = pg.transform.scale(pg.image.load("logo.png"), (210, 50)) ###завантажуємо наш логотип

colors1 = [[0, 0, 0], [24, 24, 24], [48, 48, 48], [64, 64, 64], [128, 128, 128], [155, 155, 155], [200, 200, 200], ###перша палітра кольорів
           [255, 255, 255],
           [27, 38, 49], [40, 55, 71], [46, 64, 83], [52, 73, 94], [93, 109, 126], [133, 146, 158], [174, 182, 191],
           [214, 219, 223],
           [77, 86, 86], [95, 106, 106], [113, 125, 126], [149, 165, 166], [170, 183, 184], [191, 201, 202],
           [213, 219, 219], [229, 232, 232],
           [98, 101, 103], [121, 125, 127], [144, 148, 151], [189, 195, 199], [202, 207, 210], [229, 231, 233],
           [248, 249, 249], [255, 255, 255],
           [100, 30, 22], [123, 36, 28], [146, 43, 33], [192, 57, 43], [205, 97, 85], [217, 136, 128], [230, 176, 170],
           [242, 215, 213],
           [120, 40, 31], [148, 49, 38], [176, 58, 46], [220, 76, 60], [236, 112, 99], [241, 148, 138], [245, 183, 177],
           [250, 219, 216],
           [74, 35, 90], [91, 44, 111], [108, 52, 131], [142, 68, 173], [165, 105, 189], [187, 143, 206],
           [210, 180, 222], [232, 218, 239],
           [21, 67, 96], [26, 82, 118], [31, 97, 141], [41, 128, 185], [84, 153, 199], [127, 179, 213], [169, 204, 227],
           [212, 230, 241],
           [20, 90, 50], [25, 111, 61], [34, 141, 84], [34, 174, 96], [82, 190, 128], [125, 206, 160], [169, 223, 191],
           [212, 239, 223],
           [125, 102, 8], [154, 125, 10], [183, 149, 11], [230, 196, 15], [244, 208, 63], [247, 220, 111],
           [249, 231, 159], [252, 243, 207],
           [126, 81, 9], [156, 100, 12], [185, 119, 14], [242, 156, 18], [245, 176, 65], [248, 196, 113],
           [250, 215, 160], [253, 235, 208],
           [110, 44, 0], [135, 54, 0], [160, 64, 0], [211, 84, 0], [220, 118, 51], [229, 152, 102], [237, 187, 153],
           [246, 221, 204]
           ]

colors2 = [[241, 157, 154], [241, 179, 164], [246, 209, 190], [252, 225, 213], [242, 193, 173], [241, 175, 153], ###друга пальтра кольорів
           # Skins
...
g1 = Grid(62,62, 12, 0, 0, [255, 255, 255])###розташування та розмір кнопок збереження
save_b = Button(20, 745, 69, 25, (100, 100, 100), "Save", 1, 20, (255, 255, 255))
load_b = Button(110, 745, 69, 25, (100, 100, 100), "Load", 1, 20, (255, 255, 255))
export_b = Button(200, 745, 69, 25, (100, 100, 100), "Export", 1, 20, (255, 255, 255))
SL_Buttons = [save_b, load_b, export_b]

S_brushSize = Button(880, 305, 10, 20, (240, 240, 240), "Brush Size", 2)  ###налаштовуємо підписи для повзунків
S_eraserSize = Button(880, 225, 10, 20, (240, 240, 240), "Eraser Size", 2)
S_buttons = [S_brushSize, S_eraserSize]

B_penTool = Button(825, 60, 30, 30, (80, 80, 80), "", 1)   ###робимо кнопки для інструментів
...
fileFont = pg.font.SysFont(None, 23)
nameSurface = pg.Surface((370, 20))
nameSurface.fill(pg.Color("White"))  ###віконечко для відображення ім'я файлу
fileName = "unnamed"
###налагаджуємо процес вибору інструментів та кольору
selectedTool = 0
...
def draw_palette(scheme): ###налагоджуємо палітри кольорів
    screen.blit(colorTitle, (779, 380))
    pg.draw.rect(screen, (200, 200, 200), (779, 400, 170, 340))
...
def paint(var): ###функція для малювання, прописуємо дії при різних написканнях миші на інструменти
    ...
    if sizeToDraw == 1:
        mouseRelPosX = max(penSize - 1, min(g1.xCount - 1, int(
            Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pg.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 1, min(g1.yCount - 1, int(
            Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pg.mouse.get_pos()[1]))))
    ...
    if sizeToDraw == 3:
        ...
        g1.change_color(mouseRelPosY + 1, mouseRelPosX - 1, colorUsing)
    if sizeToDraw == 4:
        mouseRelPosX = max(penSize - 2, min(g1.xCount - 3, int(
            Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pg.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 2, min(g1.yCount - 3, int(
            Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pg.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY - 1, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX - 1, colorUsing)
        ...
        g1.change_color(mouseRelPosY - 2, mouseRelPosX, colorUsing)
    if sizeToDraw == 5:
        mouseRelPosX = max(penSize - 3, min(g1.xCount - 3, int(
            Remap(0, (g1.cellSize * g1.xCount), 0, g1.xCount, pg.mouse.get_pos()[0]))))
        mouseRelPosY = max(penSize - 3, min(g1.yCount - 3, int(
            Remap(0, (g1.cellSize * g1.yCount), 0, g1.yCount, pg.mouse.get_pos()[1]))))
        g1.change_color(mouseRelPosY, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY - 1, mouseRelPosX, colorUsing)
        g1.change_color(mouseRelPosY, mouseRelPosX - 1, colorUsing)
...
def fill(gridObject, posX, posY, colorNow, fillColor): ###функція заливки
    if posX < 0 or posY < 0:
        return
...
def tool_activate(toolIndex):###вибір кольору різними інстументами
    global colorUsing
    if toolIndex == 0:
...
def Capture(display, name, pos, size):  ### (pygame Surface, String, tuple, tuple)
    image = pg.Surface(size)  ### створюємо поверхню зображення
    image.blit(display, (0, 0), (pos, size))  ###Blit частина дисплея до зображення
    pg.image.save(image, name)  ### зберігаємо зображення на диск


def FileManager(var): ###функція для роботи з файлами
...
def SaveFile(gridObject, filePath): ###налагоджуємо збереження файлу
    global fileName
    ...
    if filePath:
        ...
        pg.display.set_caption("Pyint  - " + fileName)


def OpenFile(filePath): ### налагоджуємо завантаження файлу в додаток
    global g1, fileName
...
def key_event_up(event):###налагоджуємо події гарячих клавіш
...
while True: ###основний цикл виконання програми
    clock.tick(240)
    ...
    screen.blit(nameSurface, (310, 750))
    nameText = fileFont.render(fileName, True, (0, 0, 0))
    screen.blit(nameText, (320, sh - 95))
    screen.blit(logo, (747, 745))
